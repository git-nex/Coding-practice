...
void solve()
{
  ...
  fo(i,1,l+1,1)
  {
    for(j=0;j+(1<<i)<=n;j++)
    {
      sparsematrix[i][j]=max(sparsematrix[i-1][j],sparsematrix[i-1][j+(1<<i-1)]);
    }
  }
}
...
int query(int x,int y)
{
  ...
  ll kk=L[y-x+1];
  return max(sparsematrix[kk][x],sparsematrix[kk][y-(1<<kk)+1]);
}
...
int32_t main()
{
    ...
    for(k=0;k<t;k++)
    {
        ll m,i,j,count=0,x,ans=0,c1=0,c2=0,y;
        bool flag=true;
        vll a;
        string s;
        read(n);
