...
vll sdw;
void bakctrack(vector<ll> &a , vector<ll> &temp , int index , ll &target , ll &up , ll &bo)
{
  
  ll sum=0;
  if(temp.size()>0 and temp.size()<a.size())
  {
  ...
  ll x=__gcd(sum,(target-sum));
  //dbgm(sum,target-sum);
  ll sum2=sum/x;
  ll tar=(target-sum)/x;
  
  if(sum2==up and tar==bo)
  {
    //dbgm(sum2,tar);
    //dbg(x);
    if(sdw.size()>0)
  ...
  {
    temp.pb(a[i]);
    bakctrack(a,temp,i+1,target,up,bo);
    temp.pop_back();
  }
}
