...
int32_t main()
{
    ...
    for(k=0;k<t;k++)
    {
        ll n,m,i,j,count=0,x,ans=0,c1=0,c2=0,y,kk,l;
        bool flag=true;
        //vll a,b;
        string s;
        read(n,m);
        vector<vector<ll> > a(n,vector<ll>(m,(ll)1e14));
        fo(i,0,n,1)
        {
          fo(j,0,m,1)
          {
        ...
        for(j=0;j<m;j++)
        {
            ...
            dp[0][j]=2*(abs(a[0][0]-a[0][j])+abs(a[0][m-1]-a[0][j]));
        }
        for(i=1;i<n;i++)
        {
          for(j=0;j<m;j++)
          {
            for(l=0;l<m;l++)
            {
              dp[i][j] = min(dp[i][j],abs(a[i-1][l]-a[i][j])+2*(abs(a[i][0]-a[i][j])+abs(a[i][m-1]-a[i][j])));
            }
          }
        }
