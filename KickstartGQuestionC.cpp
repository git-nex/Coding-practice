...
int32_t main()
{
    ...
    for(k=0;k<t;k++)
    {
        ll n,m,i,j,count=0,x,c1=0,c2=0,y;
        bool flag=true;
        pll ans;
        string s;
        read(n);
        ...
        fo(j,-100,101,1)
        {
          fo(i,-100,101,1)
          {
            ...
            {
              ll p,p1,p2,p3,p4;
              //dbgm(a[l][0],a[l][1],a[l][2],a[l][3]);
              if(a[l][0]<=i and i<=a[l][2] and a[l][1]<=j and j<=a[l][3])
              {
                
                p=0;
              }
              else if(a[l][0]<=i and i<=a[l][2])
              {
                p=min(abs(j-a[l][1]),abs(j-a[l][3]));
              }
              else if(a[l][1]<=j and j<=a[l][3])
              {
                p=min(abs(i-a[l][0]),abs(i-a[l][2])); 
              }
              else{
              p1=abs(a[l][0]-i)+abs(a[l][1]-j);
              p2=abs(a[l][0]-i)+abs(a[l][3]-j);
              p3=abs(a[l][2]-i)+abs(a[l][1]-j);
              p4=abs(a[l][2]-i)+abs(a[l][3]-j);
              p=min(p1,min(p2,min(p3,p4)));}
              count+=p;
              //dbg(p);
            }
