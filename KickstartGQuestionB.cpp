...
int32_t main()
{
    ...
    for(k=0;k<t;k++)
    {
        ll n,m,i,j,count=0,x,ans=0,c1=0,c2=0,y,l=0,r=0;
        bool flag=true;
        vll a;
        vpll b;
        map<ll,set<ll>> mp;
        ...
        fo(i,0,n,1)
        {
          ...
          fo(j,i,n,1)
          {
            ...
            if(sum==m)
            {
              ...
              {
                auto it=upper_bound(mp[m-sum].begin(),mp[m-sum].end(),j);
                if(it!=mp[m-sum].end())
                {
                  for(auto it2=it;it2!=mp[m-sum].end();it2++)
                  {
                    ans=min(ans,j-i+1+mp2[m-sum][*it2]);
                  }
