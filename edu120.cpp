...
int32_t main()
{
    ...
    for(k=0;k<t;k++)
    {
        ll n,m,i,j,count=0,x,ans=0,c1=0,c2=0,y,kk,sum=0;
        bool flag=true;
        vll a;
        string s;
        read(n,kk);
        ...
        suffix_sum[n-2]=a[n-1]-a[0];
        FO(i,n-3,1,-1)
        {
          
          suffix_sum[i]+=suffix_sum[i+1]+a[i+1]-a[0];
        }
        fo(i,0,n-1,1)
        {
          cout<<suffix_sum[i]<<" ";
        ...
        fo(i,1,sqrt(sum)+1,1)
        {
          ...
          while(low<high)
          {
            ...
            x=sum-suffix_sum[mid]-(n-1-mid+1)*i;
            dbgm(mid,x);
            if(x<=kk)
            {
              low=mid+1;
          ...
          dbgm(i,low); 
          ll ki = (ceil)((double)sum-suffix_sum[low]-kk/(double)i);
          ans=min(ans,i+ki-1); 
        }
        cout<<ans;
        cout<<endl;
